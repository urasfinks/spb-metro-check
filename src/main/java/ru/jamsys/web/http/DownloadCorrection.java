...
public class DownloadCorrection implements PromiseGenerator, HttpHandler {
    ...
    public Promise generate() {
        return servicePromise.get(index, 700_000L)
                ...
                .then("generateCsv", (_, promise) -> {
                    ...
                    String direction = input.getHttpRequestReader().getMap().getOrDefault("direction", "return");
                    HttpServletResponse response = input.getResponse();

                    response.setContentType("text/csv");
                    response.addHeader("Content-Disposition", "attachment;filename=" + getUniqueFileName(direction));
                    ...
                    switch (direction) {
                        case "correction":
                            csvWriter.writeNext(getLineCorrectionFirstLine());
                            break;
                        case "return":
                    ...
                    String dateDoc = getDate2();
                    result.forEach(stringObjectMap -> {
                        switch (direction) {
                            case "correction":
                                csvWriter.writeNext(getLineCorrection(
                                        stringObjectMap,
                                        counter,
                                        dateDoc,
                                        18,
                                ...
                                break;
                            case "return":
                                csvWriter.writeNext(getLineReturn(
                                        stringObjectMap,
                                        counter,
                                        dateDoc,
                                        18,
    ...
    public String getDate2() {
        SimpleDateFormat dtf = new SimpleDateFormat("yyyy-MM-dd'T'00:00:00");
        return dtf.format(new Date());
    }

    public String[] getLineCorrectionFirstLine() {
        return new String[]{"", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", ""};
    }

    public String[] getLineCorrection(Map<String, Object> row, AtomicInteger counter, String dateCorrection, int numDoc, Map<String, String> station) {
    ...
    public String[] getLineReturn(Map<String, Object> row, AtomicInteger counter, String dateCorrection, int numDoc, Map<String, String> station) {
        String complexCode = "" + row.get("code") + row.get("gate");
        return new String[]{
                counter.incrementAndGet() + "",
                "2",
